  
const expressJwt = require('express-jwt');
const koaJwt = require('koa-jwt')
const userService = require('../microservices/users/users.service');
const config = require('../config.json');

module.exports = jwt;

//expressJwt(...) returns a function that takes three paramaters req, res and next. Thus, this will register as middleware.
function jwt() {
const secret = config.secret;
//   return koaJwt({secret})
    return koaJwt({ secret , isRevoked }).unless({
        path: [
            // public routes that don't require authentication
            '/',
            '/users/register',
            '/users',
            '/users/authenticate'
        ]
    });
}


async function isRevoked(ctx, decodedToken, done) {
   // console.log("isRevoked():", req.body, payload);

    const user = await userService.getByUsername(decodedToken.sub);



   // console.log("user in JWT",user);
    // revoke token if user no longer exists
    if (!user) {

        return done(null, true);
    }

    // done (Function) - A function with signature function(err, secret) to be invoked when the secret is retrieved.
    done();
};